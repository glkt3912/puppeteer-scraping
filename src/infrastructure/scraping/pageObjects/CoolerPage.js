  ...
  async getData() {
    ...
    return this.page.evaluate(() => {
      ...
      const items = Array.from(nodeList).map((node) => {
        ...
        const type = extractTextByRegex(text, /タイプ：(.+?)(?=  幅x高さx奥行：|$)/);
        const size = extractTextByRegex(text, /幅x高さx奥行：(.+?)(?= TDP：|$)/);
        const tdp = extractTextByRegex(text, /TDP：(.+?)(?= 最大ノイズレベル：|$)/);
        const noiseMaxL = extractTextByRegex(text, /最大ノイズレベル：(.+?)(?=\n|$)/);
