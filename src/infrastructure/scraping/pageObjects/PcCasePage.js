  ...
  async getData() {
    ...
    return this.page.evaluate(() => {
      ...
      const items = Array.from(nodeList).map((node) => {
        ...
        const formFactor = extractTextByRegex(text, /対応マザーボード：(.+?)(?= 幅x高さx奥行：|$)/);
        const size = extractTextByRegex(text, /幅x高さx奥行：(.+?)(?= 3.5インチベイ：|$)/);
        const mountingBracket35 = extractTextByRegex(text, /3.5インチベイ：(.+?)(?= 3.5インチシャドウベイ：|$)/);
        const mountingBracket35shadow = extractTextByRegex(text, /3.5インチシャドウベイ：(.+?)(?= 5.25インチベイ：|$)/);
        const mountingBracket525 = extractTextByRegex(text, /5.25インチベイ：(.+?)(?=\n|$)/);
