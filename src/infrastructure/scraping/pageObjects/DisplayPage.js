  ...
  async getData() {
    ...
    return this.page.evaluate(() => {
      ...
      const items = Array.from(nodeList).map((node) => {
        ...
        const size = extractTextByRegex(text, /モニタサイズ：(.+?)(?= モニタタイプ：|$)/);
        const displayType = extractTextByRegex(text, /モニタタイプ：(.+?)(?= 解像度（規格）：|$)/);
        const resolution = extractTextByRegex(text, /解像度（規格）：(.+?)(?= 入力端子：|$)/);
        const displayInput = extractTextByRegex(text, /入力端子：(.+?)(?= パネル種類：|$)/);
        const panelType = extractTextByRegex(text, /パネル種類：(.+?)(?= リフレッシュレート\(垂直走査周波数\)：|$)/);
        const refreshRate = extractTextByRegex(text, /リフレッシュレート\(垂直走査周波数\)：(.+?)(?=\n|$)/)
        ...
        return {
          name, brand, releaseDate, price, size, displayType, resolution, displayInput, panelType, refreshRate, imgSrc
        };
      });
      return items;
    });
